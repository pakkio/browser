...
async function serveVideoThumbnail(req, res) {
    ...
    try {
        ...
        const probeInfo = JSON.parse(probeResult.stdout);
        const duration = parseFloat(probeInfo.format?.duration) || 0;

        if (duration === 0) {
            return res.status(400).send('Could not determine video duration');
        }
...
async function serveDirectoryLibrary(req, res) {
    ...
    try {
        const entries = await fsp.readdir(dirPath, { withFileTypes: true });
        const filteredEntries = entries.filter(entry => entry.name !== '.browser' && !entry.isDirectory());
