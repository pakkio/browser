...
function renameAnnotation(oldPath, newPath) {
    try {
        // Load existing annotations for the old file's directory
        const oldDirAnnotations = loadDirectoryAnnotations(oldPath);
        
        // Check if there's an annotation for the old path
        const annotation = oldDirAnnotations[oldPath];
        if (!annotation) {
            // No annotation to migrate
            return true;
        }
        
        // Check if old and new paths are in the same directory
        const oldDir = path.dirname(oldPath);
        const newDir = path.dirname(newPath);
        
        if (oldDir === newDir) {
            // Same directory - just rename the key
            delete oldDirAnnotations[oldPath];
            oldDirAnnotations[newPath] = {
                ...annotation,
                lastModified: new Date().toISOString()
            };
            return saveDirectoryAnnotations(newPath, oldDirAnnotations);
        } else {
            // Different directories - delete from old, add to new
            delete oldDirAnnotations[oldPath];
            saveDirectoryAnnotations(oldPath, oldDirAnnotations);
            
            // Load annotations for the new directory and add
            const newDirAnnotations = loadDirectoryAnnotations(newPath);
            newDirAnnotations[newPath] = {
                ...annotation,
                lastModified: new Date().toISOString()
            };
            return saveDirectoryAnnotations(newPath, newDirAnnotations);
        }
    } catch (error) {
...
function searchAnnotations(req, res) {
    try {
        const { query, hasComment, hasStars, hasColor, hasBookmarks, color, fileType } = req.query;
...
// Bookmark functions for videos
function saveBookmarks(filePath, bookmarks) {
    try {
        // Load existing annotations for this directory
        const dirAnnotations = loadDirectoryAnnotations(filePath);

        // Update or create annotation with bookmarks
        if (!dirAnnotations[filePath]) {
            dirAnnotations[filePath] = {};
        }

        // Keep existing annotation data and add/update bookmarks
        dirAnnotations[filePath].bookmarks = bookmarks;
        dirAnnotations[filePath].lastModified = new Date().toISOString();
...
function getBookmarks(filePath) {
    try {
        const dirAnnotations = loadDirectoryAnnotations(filePath);
        return dirAnnotations[filePath]?.bookmarks || [];
    } catch (error) {
