            ...
            zipfile.on('entry', (entry) => {
                if (entry.fileName === filePath) {
                    zipfile.openReadStream(entry, (err, readStream) => {
                        ...
                        const ext = path.extname(filePath).toLowerCase();
                        const { getImageMimeType, getVideoMimeType } = require('./config');
                        const imageMimeType = getImageMimeType(filePath);
                        const videoMimeType = getVideoMimeType(filePath);
                        ...
                        if (isTextExtension(ext)) {
                        ...
                        } else if (imageMimeType || videoMimeType) {
                            // Images and videos - serve as binary with proper mime type
                            const mimeType = imageMimeType || videoMimeType;
                            ...
                            // Other binary files - hex dump mode
                            const chunks = [];
                            let bytesRead = 0;
