...
Respond in JSON format:
{
  ...
  "summary": "Brief overview of what was found that matches the user's request"
            ...
            // Use simple JSON mode for all models to avoid compatibility issues
    ...
    async summarizeFileContent(filePath = null, content = null, fullPdf = false) {
        return new Promise(async (resolve, reject) => {
            ...
            if (!apiKey) {
                resolve({
                    summary: "AI summary is not available. Please configure your OPENROUTER_API_KEY in the .env file to enable AI-powered summaries.",
                    stats: { model: "none", total_time: 0, input_tokens: 0, output_tokens: 0, cached: false }
                });
                return;
            }
            ...
            if (!content && !filePath) {
                reject(new Error('Either file path or content must be provided'));
                return;
            }
            ...
            if (fullPdf && filePath && path.extname(filePath).toLowerCase() === '.pdf') {
                try {
                    ...
                    resolve({
                        ...
                        stats: { model: "none", total_time: 0, input_tokens: 0, output_tokens: 0, cached: false, error: error.message }
                    });
                    return;
                }
            }
...
Respond with just the summary text, no additional formatting.`;
            ...
            req.on('error', (error) => {
                console.error('API request error:', error);
                resolve({
                    summary: `Error during AI summarization: ${error.message}`,
                    stats: { 
                        model: model, 
                        total_time: (Date.now() - startTime) / 1000, 
                        input_tokens: 0, 
