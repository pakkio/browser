...
async function preprocessEpubForPdf(epubFilePath) {
    ...
    return new Promise((resolve, reject) => {
        ...
        yauzl.open(epubFilePath, { lazyEntries: true }, async (err, zipfile) => {
            ...
            zipfile.on('end', async () => {
                console.log(`[${new Date().toISOString()}] 📊 Found ${totalEntries} files to process`);
                
                // Process each entry with better error handling
                for (const entry of entriesList) {
                    try {
                        const content = await new Promise((entryResolve, entryReject) => {
                            const timeoutHandle = setTimeout(() => {
                                entryReject(new Error('Entry processing timeout'));
                            }, 30000); // 30 second timeout per entry
                            
                            zipfile.openReadStream(entry, (streamErr, readStream) => {
                                clearTimeout(timeoutHandle);
                                
                                if (streamErr) {
                                    entryReject(streamErr);
                                    return;
                                }
                                
                                const chunks = [];
                                readStream.on('data', chunk => chunks.push(chunk));
                                readStream.on('end', () => entryResolve(Buffer.concat(chunks)));
                        ...
                        console.error(`[${new Date().toISOString()}] ❌ Error processing ${entry.fileName}: ${entryErr.message}`);
                        processingErrors++;
                ...
                if (corruptedImages.size === 0) {
                    ...
                    console.log(`[${new Date().toISOString()}] ✅ No corrupted images found, using original EPUB`);
                    cleanup();
                    resolve(epubFilePath);
                    return;
                }
...
async function convertEpubToPdf(epubFilePath) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        // Use pandoc with pdflatex - works for most EPUBs
        // Add Unicode math support via LaTeX packages
        const pandoc = spawn('pandoc', [
            ...
            '-V', 'fontsize=12pt',
            '-V', 'linestretch=1.3',
            // Add packages for Unicode math symbols and robust image handling
            '-V', 'header-includes=\\usepackage{textcomp}',
            ...
            '-V', 'header-includes=\\DeclareUnicodeCharacter{221E}{$\\infty$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{223C}{$\\sim$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2248}{$\\approx$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2260}{$\\neq$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2264}{$\\leq$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2265}{$\\geq$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{00B1}{$\\pm$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{00D7}{$\\times$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{00F7}{$\\div$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2190}{$\\leftarrow$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2192}{$\\rightarrow$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2194}{$\\leftrightarrow$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2202}{$\\partial$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2207}{$\\nabla$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2211}{$\\sum$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{221A}{$\\sqrt{}$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{222B}{$\\int$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2229}{$\\cap$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{222A}{$\\cup$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2227}{$\\wedge$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2228}{$\\vee$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{00AC}{$\\neg$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2200}{$\\forall$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2203}{$\\exists$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2208}{$\\in$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2209}{$\\notin$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2282}{$\\subset$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2283}{$\\supset$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2286}{$\\subseteq$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2287}{$\\supseteq$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{03B1}{$\\alpha$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{03B2}{$\\beta$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{03B3}{$\\gamma$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{03B4}{$\\delta$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{03B5}{$\\epsilon$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{03C0}{$\\pi$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{03C3}{$\\sigma$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{03C9}{$\\omega$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{0394}{$\\Delta$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{03A3}{$\\Sigma$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{03A9}{$\\Omega$}',
            // Common punctuation and quotation marks that cause LaTeX errors
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2018}{`}', // Left single quotation mark
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2019}{\\textquotesingle}', // Right single quotation mark (apostrophe)
            '-V', 'header-includes=\\DeclareUnicodeCharacter{201C}{``}', // Left double quotation mark
            '-V', 'header-includes=\\DeclareUnicodeCharacter{201D}{\\textquotedbl}', // Right double quotation mark
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2013}{--}', // En dash
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2014}{---}', // Em dash
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2026}{\\ldots}', // Horizontal ellipsis
            '-V', 'header-includes=\\DeclareUnicodeCharacter{00A0}{ }', // Non-breaking space
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2020}{\\textdagger}', // Dagger
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2021}{\\textdaggerdbl}', // Double dagger
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2022}{\\textbullet}', // Bullet
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2030}{\\textperthousand}', // Per mille sign
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2032}{\\textquotesingle}', // Prime (single quote)
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2033}{\\textquotedbl}', // Double prime
            // Unicode space characters that commonly cause LaTeX errors
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2002}{ }', // En Quad
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2003}{ }', // Em Quad  
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2004}{ }', // Three-Per-Em Space
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2005}{ }', // Four-Per-Em Space
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2006}{ }', // Six-Per-Em Space
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2007}{ }', // Figure Space
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2008}{ }', // Punctuation Space
            '-V', 'header-includes=\\DeclareUnicodeCharacter{2009}{ }', // Thin Space
            '-V', 'header-includes=\\DeclareUnicodeCharacter{200A}{ }', // Hair Space
            '-V', 'header-includes=\\DeclareUnicodeCharacter{200B}{}',  // Zero Width Space (remove it)
            '-V', 'header-includes=\\DeclareUnicodeCharacter{202F}{ }', // Narrow No-Break Space
            '-V', 'header-includes=\\DeclareUnicodeCharacter{205F}{ }', // Medium Mathematical Space
            // Common Private Use Area characters (custom bullets/symbols)
            '-V', 'header-includes=\\DeclareUnicodeCharacter{F0B7}{$\\bullet$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{F0A7}{$\\circ$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{F076}{$\\star$}',
            '-V', 'header-includes=\\DeclareUnicodeCharacter{F0FC}{$\\triangleright$}',
