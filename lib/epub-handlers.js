...
async function validateImage(imagePath) {
    ...
    return new Promise((resolve) => {
        ...
        // Use ImageMagick's identify command to check image validity
        const identify = spawn('identify', ['-ping', imagePath]);
        
        let isValid = false;
        let stderr = '';
        
        identify.stdout.on('data', (data) => {
            // If identify successfully outputs image info, the image is valid
            if (data.toString().includes(imagePath)) {
                isValid = true;
            }
        });
        ...
        identify.on('close', (code) => {
            if (code === 0 && isValid) {
                ...
                console.log(`[${new Date().toISOString()}] ❌ Image is corrupted: ${imagePath} - ${stderr}`);
                resolve(false);
            }
        });
...
async function preprocessEpubForPdf(epubFilePath) {
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            return reject(new Error(`Failed to create temp directory: ${err.message}`));
        }
        
        yauzl.open(epubFilePath, { lazyEntries: true }, async (err, zipfile) => {
            if (err) {
                return reject(new Error(`Failed to open EPUB: ${err.message}`));
            }
            ...
            zipfile.on('end', async () => {
                    ...
                    try {
                        ...
                        if (isImage) {
                            ...
                            try {
                                ...
                                const isValid = await validateImage(tempImagePath);
                                if (!isValid) {
                ...
                // Create new EPUB without corrupted images using a simple ZIP approach
                try {
                    const tempEpubPath = path.join(tempDir, 'cleaned.epub');
                    const { spawn } = require('child_process');
                    
                    // Use system zip command to create the new EPUB
                    const zipFiles = [];
                    ...
                    zip.on('close', (code) => {
                        if (code === 0) {
                            console.log(`[${new Date().toISOString()}] ✅ Created cleaned EPUB: ${tempEpubPath}`);
                            resolve(tempEpubPath);
                        } else {
        ...
        pandoc.on('close', (code) => {
            ...
            if (code !== 0) {
                ...
                if (stderr.includes('Unicode character') || stderr.includes('not set up for use with LaTeX')) {
                    // Extract the Unicode character if possible
                    const unicodeMatch = stderr.match(/Unicode character (.+?) \(U\+([0-9A-F]+)\)/);
                    if (unicodeMatch) {
                        errorMessage = `This EPUB contains Unicode character "${unicodeMatch[1]}" (U+${unicodeMatch[2]}) that cannot be converted to PDF. The EPUB may contain special symbols or non-Latin characters that are not supported by the PDF converter.`;
                    } else {
                ...
                } else if (stderr.includes('Division by 0') || stderr.includes('graphics Error')) {
                    errorMessage = 'This EPUB contains images with invalid dimensions that cannot be processed. The EPUB file may be corrupted or use an unsupported image format.';
                    ...
                    const shortError = stderr.substring(0, 200);
                    errorMessage = `PDF conversion failed: ${shortError}${stderr.length > 200 ? '...' : ''}`;
                }
