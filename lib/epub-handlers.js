...
async function convertEpubToPdf(epubFilePath) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        // Use pandoc with xelatex - supports CJK and all Unicode characters natively
        const pandoc = spawn('pandoc', [
            ...
            '-V', 'fontsize=12pt',
            '-V', 'linestretch=1.3',
            // Add packages for CJK support and robust image handling
            '-V', 'header-includes=\\usepackage{xeCJK}',
            '-V', 'header-includes=\\usepackage{fontspec}',
            '-V', 'header-includes=\\usepackage{graphicx}',
            '-V', 'header-includes=\\usepackage{grffile}',
            // Set CJK fonts - using commonly available system fonts
            // These fonts should be available on most Linux systems
            '-V', 'header-includes=\\setCJKmainfont{Noto Sans CJK SC}',
            '-V', 'header-includes=\\setCJKsansfont{Noto Sans CJK SC}',
            '-V', 'header-includes=\\setCJKmonofont{Noto Sans Mono CJK SC}',
